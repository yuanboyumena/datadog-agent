...
int sys_enter(struct _tracepoint_raw_syscalls_sys_enter *args) {
    ...
    struct bpf_map_def *noisy_processes = select_buffer(SYSCALL_MONITOR_KEY, &noisy_processes_fb, &noisy_processes_bb);
...
int sys_exit(struct _tracepoint_raw_syscalls_sys_exit *args) {
    ...
    if (*concurrent_syscalls_counter) {
        __sync_fetch_and_add(concurrent_syscalls_counter, -1);
    }
...
int sched_process_exec(struct _tracepoint_sched_sched_process_exec *ctx) {
    ...
    struct bpf_map_def *exec_count = select_buffer(SYSCALL_MONITOR_KEY, &exec_count_fb, &exec_count_bb);
    if (exec_count == NULL)
        return 0;
